...
// Return the best offer value that matches the header values.
//
...
func (n *Negotiator) Match(headerValues []string) (value string, ok bool) {
	...
	if len(matches) == 0 {
		...
		// No match; default to the first offer with false to indicate no match
...
// Return all offers that match the header values, sorted (stably) by quality.
func (n *Negotiator) MatchAll(headerValues []string) []string {
	...
	matches := slices.SortedStableFunc(slices.Values(n.Offers), func(a, b string) int {
